...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("client.events.store") }}" enctype="multipart/form-data">
            @csrf
                ...
                <div class="form-group col-md-3">
                    ...
                    <select class="form-control select2 {{ $errors->has('government') ? 'is-invalid' : '' }}" name="government_id" id="government_id" required>
                        @foreach($governments as $id => $entry)
                            <option value="{{ $id }}" {{ old('government_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-6">
                    ...
                    <select class="form-control select2 {{ $errors->has('cawaders') ? 'is-invalid' : '' }}" name="cawaders[]" id="cawaders" multiple>
                        @foreach($cawaders as $id => $cawader)
                            <option value="{{ $id }}" {{ in_array($id, old('cawaders', [])) ? 'selected' : '' }}>{{ $cawader }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-6">
                    ...
                    <select class="form-control select2 {{ $errors->has('available_gates') ? 'is-invalid' : '' }}" name="available_gates[]" id="available_gates" multiple required>
                        @foreach($available_gates as $id => $available_gate)
                            <option value="{{ $id }}" {{ in_array($id, old('available_gates', [])) ? 'selected' : '' }}>{{ $available_gate }}</option>
                        @endforeach
                    </select>
...
<script>
    ...
    url: '{{ route('client.events.storeMedia') }}',
    maxFilesize: 2, // MB
