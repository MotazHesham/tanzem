    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.brands.update', [$brand->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <input type="hidden" name="latitude" id="latitude" value="{{ $brand->latitude }}">
                <input type="hidden" name="longitude" id="longitude" value="{{ $brand->longitude }}">
                <div class="row">

                    <div class="form-group col-md-4">
                        <label class="required" for="title">{{ trans('cruds.brand.fields.title') }}</label>
                        <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text"
                            name="title" id="title" value="{{ old('title', $brand->title) }}" required>
                        @if ($errors->has('title'))
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select class="form-control select2 {{ $errors->has('event') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($events as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('event_id') ? old('event_id') : $brand->event->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}</option>
                            @endforeach
                        </select>
                        @if ($errors->has('event'))
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <input class="form-control {{ $errors->has('zone_name') ? 'is-invalid' : '' }}" type="text"
                            name="zone_name" id="zone_name" value="{{ old('zone_name', $brand->zone_name) }}" required>
...
@section('scripts') 
    @include('map_scripts.brands.edit')
    <script>
