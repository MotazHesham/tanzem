...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("company.cawaders.update", [$cawader->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <input type="hidden" name="user_id" value="{{ $cawader->user->id}}" id="">

            <input type="hidden" name="companies_and_institution_id" value="{{ $cawader->companies_and_institution_id }}">
            <div class="row">
...
@section('scripts')
    <script>
        Dropzone.options.photoDropzone = {
            url: '{{ route('company.cawaders.storeMedia') }}',
            maxFilesize: 2, // MB
