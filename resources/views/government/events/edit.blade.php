...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('government.events.update', [$event->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <input type="hidden" name="government_id" value="{{ $event->government->id }}">
                <div class="row">  
                    <div class="form-group col-md-3">
                        ...
                        <select class="form-control select2 {{ $errors->has('company') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($companies as $id => $entry)
                                <option value="{{ $id }}"
                                    {{ (old('company_id') ? old('company_id') : $event->company->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $entry }}</option>
                            @endforeach
                        </select>
