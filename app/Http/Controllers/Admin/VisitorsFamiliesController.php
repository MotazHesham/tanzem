...
class VisitorsFamiliesController extends Controller
{
    ...
    {
        abort_if(Gate::denies('visitors_family_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');
    ...
    {
        abort_if(Gate::denies('visitors_family_create'), Response::HTTP_FORBIDDEN, '403 Forbidden');
    ...
    public function store(StoreVisitorsFamilyRequest $request)
    {
        ...
        return redirect()->route('admin.visitors.show',$visitorsFamily->visitor_id);
    }

    public function edit(VisitorsFamily $visitorsFamily)
    {
        abort_if(Gate::denies('visitors_family_edit'), Response::HTTP_FORBIDDEN, '403 Forbidden');

        $visitors = Visitor::pluck('national', 'id')->prepend(trans('global.pleaseSelect'), '');

        $visitorsFamily->load('visitor');

        return view('admin.visitorsFamilies.edit', compact('visitors', 'visitorsFamily'));
    }
